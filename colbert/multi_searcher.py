...
class SearchModel:
    def __init__(self, checkpoint, config=None):
        ...
        self.model = Checkpoint(self.checkpoint, colbert_config=self.config)
...
class UniSearcher:
    def __init__(self, index, config=None):
        ...
        self.initial_config = ColBERTConfig.from_existing(config, Run().config)

        default_index_root = self.initial_config.index_root_
    ...
    def set_model(self, model, checkpoint_config):
        self.config = ColBERTConfig.from_existing(checkpoint_config, self.index_config, self.initial_config)
...
class MultiSearcher:
    def __init__(self, dbtype_index_dict, checkpoint, config=None):
        print_memory_stats()

        self.model_obj = SearchModel(checkpoint, config)
        self.dbtype_searcher_dict = dict()

        for dbtype, index in dbtype_index_dict.items():
            cur_searcher = UniSearcher(index, config)
            cur_searcher.set_model(self.model_obj.model, self.model_obj.checkpoint_config)
            self.dbtype_searcher_dict[dbtype] = cur_searcher

        print_memory_stats()

    def search(self, dbtype, text: str, k=10, filter_fn=None):
        return self.dbtype_searcher_dict[dbtype].search(text, k, filter_fn)

    def search_all(self, dbtype, queries: TextQueries, k=10, filter_fn=None):
        return self.dbtype_searcher_dict[dbtype].search_all(queries, k, filter_fn)
